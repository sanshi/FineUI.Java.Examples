...
/** 行样式（服务端行绑定事件/RowAttributes，路由 {@code grid/style-row-data-bound-server-row-attributes}）：服务端按行数据给行加 data-color 属性。 */
@FineUIPage("grid/style-row-data-bound-server-row-attributes")
public class StyleRowDataBoundServerRowAttributes extends PageBase {
    ...
    public void Grid1_RowDataBound(Object sender, GridRowDataBoundEventArgs e) {
        ...
        if (entranceYear >= 2000 && entranceYear <= 2004) {
            e.getRowAttributes().put("data-color", "color1");
        } else if (entranceYear == 2008) {
            e.getRowAttributes().put("data-color", "color3");
        }
    }
}
