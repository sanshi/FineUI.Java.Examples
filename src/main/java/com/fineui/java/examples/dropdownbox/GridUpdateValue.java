...
/**
 * 下拉表格（单选，后台更新选中值）演示页（路由 {@code drop-down-box/grid-update-value}）：
 * 「更新下拉框的值为[张三石]」按钮在服务端改下拉框的值，文本置空由客户端根据表格行数据重算。
 */
@FineUIPage("drop-down-box/grid-update-value")
public class GridUpdateValue extends PageBase {
    ...
    public void btnUpdateValue_Click(Object sender, EventArgs e) {
        // 1. 对于未分页的表格，可以不设置文本，以便客户端重新计算
        // 2. 对于分页的表格，一定要手工设置文本，否则客户端无法取到值对应的文本
        DropDownBox1.setValue("112");
