...
/**
 * 始终显示弹出面板（隐藏输入框）演示页（路由 {@code drop-down-box/grid-layout-always-display-hidden}）：在始终显示面板的
 * 基础上隐藏下拉框的输入框本身——此时它就退化成一个跨页保持选中项的表格，值/选中逻辑照常工作。
 */
@FineUIPage("drop-down-box/grid-layout-always-display-hidden")
public class GridLayoutAlwaysDisplayHidden extends PageBase {
