...
/**
 * 树表格 · 复选框初始值（路由 {@code grid-tree/check-box-init-value}）：默认展开 common、勾选 common 与 menu.xml
 * （初值走客户端 row-data-bound）。「获取选中的复选框」按钮经自定义事件回发、服务端写入结果标签。
 */
@FineUIPage("grid-tree/check-box-init-value")
public class CheckBoxInitValue extends PageBase {
