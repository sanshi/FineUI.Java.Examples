...
/**
 * 树表格 · 仅叶子节点可勾选（路由 {@code grid-tree/check-box-only-leaf-check}）：开启行复选框且
 * tree-only-leaf-check，仅文件（叶子）可勾选。「获取选中的复选框」按钮经自定义事件回发、服务端写入结果标签。
 */
@FineUIPage("grid-tree/check-box-only-leaf-check")
public class CheckBoxOnlyLeafCheck extends PageBase {
