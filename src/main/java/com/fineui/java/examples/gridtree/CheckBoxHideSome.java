...
/**
 * 树表格 · 隐藏部分复选框（路由 {@code grid-tree/check-box-hide-some}）：默认隐藏 basic、res 两个目录的复选框
 * （走客户端 row-data-bound 置 checkbox=false）。「获取选中的复选框」按钮经自定义事件回发、服务端写入结果标签。
 */
@FineUIPage("grid-tree/check-box-hide-some")
public class CheckBoxHideSome extends PageBase {
