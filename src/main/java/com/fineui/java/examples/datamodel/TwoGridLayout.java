...
/**
 * 数据模型多表布局（路由 {@code data-model/two-grid-layout}）：与 {@code two-grid} 数据相同，但两个表格
 * 用 VBox 布局 + 分部片段（th:replace）承载（左表在校生、右表毕业生），演示布局容器内的多 Grid。
 */
@FineUIPage("data-model/two-grid-layout")
public class TwoGridLayout extends PageBase {
