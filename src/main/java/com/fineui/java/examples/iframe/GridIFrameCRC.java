...
/**
 * 客户端渲染操作列打开 IFrame 窗体（路由 {@code iframe/grid-iframe-crc}）：上方
 * 「搜索关键词 {@code ttbSearch} + 过滤条件下拉 {@code DropDownList1}」工具栏；操作列由客户端
 * {@code RendererFunction=renderActionField} 渲染出 {@code a.editfield} / {@code a.deletefield}，
 * 编辑链接经 JS {@code openWindow(rowData)} 打开窗体（标题「详细信息 - {Name}」），删除链接弹出
 * {@code F.alert('尚未实现！')}。表格数据每次加载经 {@code autoBindGrid()} 在两张学生表间交替。
 */
@FineUIPage("iframe/grid-iframe-crc")
public class GridIFrameCRC extends PageBase {
