...
/**
 * 节点选择（保持选中节点）（路由 {@code tree/tree-select-multi-keep-current-selection}）：多选树且 keep-current-selection，
 * 无需按 Ctrl/Shift 即可累积多选；服务端读取当前多选节点列表，并可继续追加选中节点。
 */
@FineUIPage("tree/tree-select-multi-keep-current-selection")
public class TreeSelectMultiKeepCurrentSelection extends PageBase {
