...
/**
 * 自定义解析 XML（自定义属性）（路由 {@code tree/data-bind-document-custom-attributes}）：手工遍历 XML 文档创建节点，
 * 把 XML 的 {@code Highlight} 属性转化为节点的自定义 HTML 属性 {@code data-highlight}（配合页面 CSS 高亮）；
 * 点击节点回发，回填被点击节点信息（含是否高亮）。
 */
@FineUIPage("tree/data-bind-document-custom-attributes")
public class DataBindDocumentCustomAttributes extends PageBase {
    ...
    public void Tree1_NodeClick(Object sender, TreeNodeEventArgs e) {
        ...
        boolean highlight = node.getAttributes() != null && node.getAttributes().get("data-highlight") != null;
        labResult.setText(String.format("你点击了树节点：%s（%s）%s",
                node.getText(), e.getNodeID(), highlight ? " - 高亮显示" : ""));
    }
}
