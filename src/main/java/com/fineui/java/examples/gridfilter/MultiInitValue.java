...
/**
 * 表头过滤 · 多条件 + 服务端初始化过滤值（路由 {@code grid-filter/multi-init-value}）：首屏由服务端给姓名列设两条初始
 * 过滤条件（开始于「张」或结束于「国」，匹配「任一满足 any」）；「后台更新过滤数据」按钮先清空再设（包含「婷婷」或结束于「国」）。
 */
@FineUIPage("grid-filter/multi-init-value")
public class MultiInitValue extends PageBase {
