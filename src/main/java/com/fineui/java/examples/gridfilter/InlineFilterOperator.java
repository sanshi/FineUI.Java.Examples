...
/**
 * 内联表头过滤 · 自定义操作符（路由 {@code grid-filter/inline-filter-operator}）：姓名列内联过滤带操作符下拉
 * （等于/包含/开始于/结束于），过滤输入框内嵌在表头。
 */
@FineUIPage("grid-filter/inline-filter-operator")
public class InlineFilterOperator extends PageBase {
