...
/**
 * 内联表头过滤 · 服务端初始化过滤值（路由 {@code grid-filter/inline-filter-init-value}）：首屏由服务端给姓名列
 * 设初始过滤值「张」；「后台更新过滤数据」按钮先清空再设「婷婷」。因首屏做了服务端过滤，需为相关列定义 column-id。
 */
@FineUIPage("grid-filter/inline-filter-init-value")
public class InlineFilterInitValue extends PageBase {
